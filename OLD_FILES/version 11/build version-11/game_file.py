...
#end_animation = pyvidplayer2.Video(absolute_path + r'\assets\images\animation_main_view.mp4')
#UI CLASSES:
#Buttons:
class Button:
...
def main_gameLoop():
    ...
    def animation_end():
        ...
        while animation_run_var:
            end_animation.draw(screen,(0,0),False)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    end_animation.stop()
                    pygame.quit()
...
FINISH THE FINAL ANIMATION WHERE THE PLAYER GETS ATTACKED BY THE MONSTER (add a radiation fog effect, make the rover warning lights go off, add some camera shake, then improve the original camera picture to make the creature more disheveled and menacing.)
ADD THE SOUND EFFECTS TO THE GAME(CAMERA SFX, OXYGEN ALERT, FACILITY AMBIENT NOISES, MAKE BLACKOUT LOUDER )
